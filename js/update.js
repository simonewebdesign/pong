...
var update = function (modifier) {
  ...
  if (ball.pos.x >= canvas.width - ball.size) {
    p1.score++;
    music.pause();
    gameEndSound();
    reset();
  ...
  if (
    ball.pos.x <= (p1.pos.x + p1.width)
    && p1.pos.x <= (ball.pos.x + ball.size)
    && ball.pos.y <= (p1.pos.y + p1.height)
    && p1.pos.y <= (ball.pos.y + ball.size)
  ) {
    ...
    // go back on track, just in case it went too out of boundaries
    ball.pos.x = p1.pos.x + p1.width +1;
  ...
  if (
    ball.pos.x <= (p2.pos.x + p2.width)
    && p2.pos.x <= (ball.pos.x + ball.size)
    && ball.pos.y <= (p2.pos.y + p2.height)
    && p2.pos.y <= (ball.pos.y + ball.size)
  ) {
    // First of all, I need to calculate the wall normal properly.
    // I start by getting the A and B points:
    ...
    // go back on track, just in case it went too out of boundaries
    ball.pos.x = p2.pos.x - ball.size -1;
  ...
  if (ball.pos.y + ball.size >= canvas.height) {
    ...
    // go back on track, just in case it went too out of boundaries
    ball.pos.y = canvas.height - ball.size -1;
  }

  // Ball movement
  ball.pos.x += ball.velocity.x * modifier;
  ball.pos.y += ball.velocity.y * modifier;
};
